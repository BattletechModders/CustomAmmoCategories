    ...
    public static class MechBayRowGroupWidget_SetData
    {
        ...
        public static void Postfix(MechBayRowGroupWidget __instance, IMechLabDropTarget dropParent, SimGameState sim)
        {
            try
            {
                ...
                if (maxPods <= 0)
                {
                    mechBayRows[0].upgradeOverlay.SetActive(true);
                    mechBayRows[1].upgradeOverlay.SetActive(true);
                    mechBayRows[2].upgradeOverlay.SetActive(true);
                }
                else if (maxPods <= 6)
                {
                    mechBayRows[0].upgradeOverlay.SetActive(false);
                    mechBayRows[1].upgradeOverlay.SetActive(true);
                    mechBayRows[2].upgradeOverlay.SetActive(true);
                }
                else if (maxPods <= 12)
                {
                    mechBayRows[0].upgradeOverlay.SetActive(false);
                    mechBayRows[1].upgradeOverlay.SetActive(false);
                    mechBayRows[2].upgradeOverlay.SetActive(true);
                }
                else
                {
                    mechBayRows[0].upgradeOverlay.SetActive(false);
                    mechBayRows[1].upgradeOverlay.SetActive(false);
                    mechBayRows[2].upgradeOverlay.SetActive(false);
                }

                // Force a refresh against the CustomBaysUICaster
                GameObject layoutTabsGO = __instance.transform.parent.parent.parent.gameObject.FindFirstChildNamed("layout_tabs");
                CustomBaysUICaster customBaysUICaster = layoutTabsGO.GetComponent<CustomBaysUICaster>();
                if (customBaysUICaster != null)
                {
                    CustomHangarHelper.RefreshHanagarUIForConstraints(customBaysUICaster, customHangarInfo);
                }

                // Fix the UNAVAILABLE widget after the base method disables it
                //var mechBayRows = __instance.gameObject.GetComponentsInChildren<MechBayRowWidget>();
                //foreach (MechBayRowWidget mbrw in mechBayRows)
                //{
                //    bool hasActiveChildren = false;
                //    foreach (MechBayDragDropSlot mbdds in mbrw.mechDropSlots)
                //    {
                //        if (mbdds.isActiveAndEnabled) { hasActiveChildren = true; }
                //        break;
                //    }
