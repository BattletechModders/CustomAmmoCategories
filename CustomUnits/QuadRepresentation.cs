...
namespace CustomUnits {
  ...
  public static class MechMeshMerge_BuildCombinedMesh {
    public static bool Prefix(MechMeshMerge __instance, ref IDictionary ___meshBoneDict,ref Mesh ___combinedMesh0, ref Mesh ___combinedMesh1,ref int ___materialCount,ref List<Matrix4x4> ___bindPoses,ref float ___bottom, ref float ___size) {
      ...
      try {
        ...
        for (index1 = 0; index1 < childrenRenderers.Length / 2; ++index1) {
          object combineInfo;
          if (childrenRenderers[index1].gameObject.activeInHierarchy && meshBoneDict.TryGetValue(childrenRenderers[index1], out combineInfo)) {
        ...
        if ((double)___bottom < 0.0 && (double)___size < 0.0) {
          ...
          foreach (KeyValuePair<SkinnedMeshRenderer, object> keyValuePair in meshBoneDict)
