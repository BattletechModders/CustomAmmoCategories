...
namespace CustomUnits {
  ...
  public class SelectionStateCommandSpawnUnit : SelectionStateCommandTargetSinglePoint {
    ...
    public override void OnAddToStack() {
      ...
      Log.TWL(0, "SelectionStateCommandSpawnUnit OnAddToStack");
      base.OnAddToStack();
    }
    public override void OnInactivate() {
      Log.TWL(0, "SelectionStateCommandSpawnUnit.OnInactivate");
      NumPositionsLocked = 0;
      base.OnInactivate();
    }
    ...
    public override bool ProcessPressedButton(string button) {
      try {
        Log.TWL(0, "SelectionStateCommandSpawnUnit.ProcessPressedButton "+ button); 
    ...
    public override void BackOut() {
      ...
      if (this.NumPositionsLocked > 0) {
        ...
        Log.TWL(0, "SelectionStateCommandSpawnUnit.BackOut:" + this.NumPositionsLocked);
      } else {
        Log.TWL(0, "SelectionStateCommandSpawnUnit.BackOut:" + this.NumPositionsLocked);
      }
    }
  ...
  public static class SpawnHelper {
    ...
    private static void SpawnAtPos(MechDef mechDef, PilotDef newPilotDef, Vector3 pos, Team parentTeam, AbstractActor parentActor) {
      Log.TWL(0, "SpawnAtPos mechDef:" + mechDef.ChassisID + " pilotDef:" + newPilotDef.Description.Id + " team:" + parentTeam.DisplayName + " actor:" + parentActor.PilotableActorDef.ChassisID);
      try {
        ...
        Log.WL(1, "spawn success:" + newActor.PilotableActorDef.ChassisID + ":" + newActor.GUID);
    ...
    public static void SpawnAtPos(string mechDefId, Vector3 pos, Team parentTeam, AbstractActor parentActor) {
      ...
      try {
        Log.TWL(0, "SpawnAtPos mechDefId:" + mechDefId + " team:" + parentTeam.DisplayName + " actor:" + parentActor.PilotableActorDef.ChassisID);
        if (UnityGameInstance.BattleTechGame.Simulation != null) {
          SimGameState sim = UnityGameInstance.BattleTechGame.Simulation;
          Log.WL(1, "GenerateRandomPilot");
          newPilotDef = Traverse.Create(sim.PilotGenerator).Method("GenerateRandomPilot", sim.CurSystem.Def.GetDifficulty(sim.SimGameMode)).GetValue<PilotDef>();
        } else {
          List<PilotDef> pilots = new List<PilotDef>();
          Log.WL(1, "Choose random pilot:"+ parentActor.Combat.DataManager.PilotDefs.Count);
    ...
    public static void SpawnAtPos(string mechDefId, PilotDef newPilotDef, Vector3 pos, Team parentTeam, AbstractActor parentActor) {
      Log.TWL(0, "SpawnAtPos mechDefId:" + mechDefId + " pilotDef:"+newPilotDef.Description.Id+" team:"+parentTeam.DisplayName+" actor:"+parentActor.PilotableActorDef.ChassisID);
      ...
      if(dataManager.MechDefs.TryGet(mechDefId, out var newMechDef) == false) {
        Log.WL(1, mechDefId + " absent in data manager");
        return;
      }
      newMechDef.Refresh();
      ...
      Log.WL(1, "DependencyCount:" + injectedDependencyLoadRequest.DependencyCount());
      ...
      injectedDependencyLoadRequest.RegisterLoadCompleteCallback((Action)(() => {
        Log.TWL(0, "DependencyLoadSuccess mechDefId:" + mechDefId + " pilotDef:" + newPilotDef.Description.Id);
        SpawnAtPos(newMechDef, newPilotDef, pos, parentTeam, parentActor);
      }));
      injectedDependencyLoadRequest.RegisterLoadFailedCallback((Action)(() => {
        Log.TWL(0, "DependencyLoadFailed mechDefId:" + mechDefId + " pilotDef:" + newPilotDef.Description.Id);
      }));
