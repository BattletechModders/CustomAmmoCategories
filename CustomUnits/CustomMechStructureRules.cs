...
#pragma warning disable CS0252
  ...
  public static class Mech_NukeStructureLocation {
    static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions) {
      ...
      Log.WL(1, "found NukeStructureLocation.GetDependentLocation call " + MethodPos.ToString("X"));
    ...
    public static ChassisLocations GetDependentLocation(ChassisLocations location, Mech mech) {
      ...
      if(info != null) { if(info.SquadInfo.Troopers > 1) { return ChassisLocations.None; } }
  ...
  public static class Mech_DamageLocation {
    static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions) {
      Log.TWL(0, "Mech.DamageLocation Transpiler");
      MethodInfo targetMethod = typeof(MechStructureRules).GetMethod("GetPassthroughLocation", BindingFlags.Static | BindingFlags.Public);
      var replacementMethod = AccessTools.Method(typeof(Mech_DamageLocation), nameof(GetPassthroughLocation));
      List<CodeInstruction> uInstructions = new List<CodeInstruction>();
      uInstructions.AddRange(instructions);
      int MethodPos = -1;
      for (int t = 0; t < uInstructions.Count; ++t) {
      ...
      if (MethodPos < 0) {
        Log.WL(1, "can't find MechStructureRules.GetPassthroughLocation call");
        return uInstructions;
      }
      Log.WL(1, "found MechStructureRules.GetPassthroughLocation call " + MethodPos.ToString("X"));
      uInstructions.Insert(MethodPos, new CodeInstruction(OpCodes.Ldarg_0));
      return uInstructions;
    }
    public static ArmorLocation GetPassthroughLocation(ArmorLocation location, AttackDirection attackDirection, Mech mech) {
      UnitCustomInfo info = mech.GetCustomInfo();
      Log.TWL(0, "Mech_NukeStructureLocation.GetPassthroughLocation " + mech.DisplayName + " " + location+" troopers:"+(info==null?"null": info.SquadInfo.Troopers.ToString()));
      if (info != null) { if (info.SquadInfo.Troopers > 1) { return ArmorLocation.None; } }
    ...
    public static bool Prefix(Mech __instance, ChassisLocations location, Vector3 attackDirection, WeaponHitInfo hitInfo, DamageType damageType) {
      ...
      try {
        ...
        if (squad == null) {
          ...
          squad.OnLocationDestroyed(location, attackDirection, hitInfo, damageType);
        }
        return false;
      }catch(Exception e) {
        Log.TWL(0, e.ToString());
