...
#pragma warning disable CS0162 // Disable "unreachable code" warnings due to DebugLog flag
    ...
    private static void LogHitSequence<T, C>(T hitLocation, float randomRoll, C bonusLocation, float bonusLocationMultiplier, string hitTable) where T : Enum {
      try {
    ...
    private static void LogSelfHitSequence(ArmorLocation hitLocation) {
      try {
    ...
    [HarmonyPriority(Priority.VeryLow)]
    public static void LogCritResult(ICombatant __instance, Weapon weapon) {
      try {
