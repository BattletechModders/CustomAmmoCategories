...
namespace CustAmmoCategories {
    ...
    public class CombatHUDMiniMap : EventTrigger
    {
        ...
        public static float GetAngle(float cx, float cy, float lx, float ly)
        {
            ...
            if ((d.x >= 0f) && (d.y >= 0f)) { result = result - 90f; }
            else
        ...
        public void minimapTextureUpdate(float deltaTime)
        {
            try
            {
                if (minimapChangeQueue.Count == 0)
                {
                    if (minimap_changed)
                    {
                        if (minimap_change_t > 0f) { minimap_change_t -= deltaTime; }
                        else
                        {
                            minimap_change_t = 0f;
        ...
        // TODO: PERFORMANCE CHANGE - JAMIE
        // Cache camera variables to remove calls to FindMainCamera, expensive and recommended to cache by Unity
        private static GameObject mainCameraCache { get; set; } = null;
        private static CameraControl cameraInstanceCache { get; set; } = null;
        public void Update()
        {
            ...
            try
            {
                ...
                foreach (AbstractActor unit in allActors)
                {
                    ...
                    if (actorsPoints.TryGetValue(unit, out var mark) == false)
                    {
                        ...
                        if (unit.Combat.LocalPlayerTeam == unit.team) { mark.detectedColor = Color.green; mark.isFriendly = true; }
                        else
                        if (unit.Combat.LocalPlayerTeam.IsEnemy(unit.team)) { mark.detectedColor = Color.red; mark.isFriendly = false; }
                        else
                ...
                if (camera != null)
                {
                    ...
                    // TODO: PERFORMANCE CHANGE - JAMIE
                    // Camera caching for performance
                    if (mainCameraCache == null)
                    {
                        mainCameraCache = Camera.main.gameObject;
                    }

                    if(cameraInstanceCache== null)
                    {
                        cameraInstanceCache = CameraControl.Instance;
                    }

                    // Camera positions utilizing cache.                    
                    Vector3 cameraPos = cameraInstanceCache.CameraPos;
                    Vector3 cameraLookPos = cameraPos + mainCameraCache.transform.forward * 100f;
